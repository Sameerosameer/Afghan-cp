...
"""%s
 ...
 - facebook    : Nadaroom
 - fanspage    : Nadaroom
 - whatsap     : 0093700514406
 - github      : github.com/HemtKhan
 - script name : bff-2
 - version     : 1.1
...
def masuk():
    ...
    if rom in(""):
    	...
    	romz = raw_input('%s [ua] Token : %s'%(P,K))
        if romz in(""):
        	print("%s [!] Isi yang benar kentod "%(M));exit()
    	try:
        ...
        if nanya in(""):
        	...
        	print ("\n%s [!] anda sungguh tolol "%(M));jeda(2);os.system("xdg-open https://t.me/hemt_afghan");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar kentod "%(M));exit()
...
def menu():
    ...
    if unik == '':
        ...
        os.system("xdg-open https://https://t.me/hemt_afghan")
    elif unik in['8','08']:
        print(ingfo)
    elif unik in['0','00']:
        print ('')
...
class ngentod:
    ...
    'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8,ps;q=0.7',
